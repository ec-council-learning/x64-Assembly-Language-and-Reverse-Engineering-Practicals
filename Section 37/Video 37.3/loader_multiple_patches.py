...
# This script allows you to do multiple file patches.
...
DATA_TO_FIND_1 = '75 62 68 41 03 00 00'
DATA_TO_REPLACE_1 = 'EB'

DATA_TO_FIND_2 = '0F 85 9F 01 00 00 6A FF'
DATA_TO_REPLACE_2 = 'E9 A0 01 00 00 90'

DATA_TO_FIND_3 = '0F 84 A2 00 00 00 6A FF'
DATA_TO_REPLACE_3 = 'E9 A3 00 00 00 90'

# Convert the space-separated hex strings to bytes
data_to_find_bytes_1 = bytes(int(x, 16) for x in DATA_TO_FIND_1.split())
data_to_replace_bytes_1 = bytes(int(x, 16) for x in DATA_TO_REPLACE_1.split())

data_to_find_bytes_2 = bytes(int(x, 16) for x in DATA_TO_FIND_2.split())
data_to_replace_bytes_2 = bytes(int(x, 16) for x in DATA_TO_REPLACE_2.split())

data_to_find_bytes_3 = bytes(int(x, 16) for x in DATA_TO_FIND_3.split())
data_to_replace_bytes_3 = bytes(int(x, 16) for x in DATA_TO_REPLACE_3.split())
...
def patch_memory(process_handle, data_to_find_bytes, data_to_replace_bytes):
...
def main(process_identifier):
    ...
    # Optional: Add a 3 secs delay to ensure the application has started and unpacked itself
    time.sleep(3)  # Adjust the delay as necessary
    ...
    # Patch the memory
    patch_memory(process_handle, data_to_find_bytes_1, data_to_replace_bytes_1)
    patch_memory(process_handle, data_to_find_bytes_2, data_to_replace_bytes_2)
    patch_memory(process_handle, data_to_find_bytes_3, data_to_replace_bytes_3)
